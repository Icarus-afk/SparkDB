...
func (h *Handler) HandlePrometheus(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Fprintf(w, "# HELP vaultlite_uptime_seconds Server uptime\n")
	fmt.Fprintf(w, "# TYPE vaultlite_uptime_seconds gauge\n")
	fmt.Fprintf(w, "vaultlite_uptime_seconds %f\n", stats.UptimeSeconds)
	fmt.Fprintf(w, "# HELP vaultlite_queries_total Total queries executed\n")
	fmt.Fprintf(w, "# TYPE vaultlite_queries_total counter\n")
	fmt.Fprintf(w, "vaultlite_queries_total %d\n", stats.TotalQueries)
	fmt.Fprintf(w, "# HELP vaultlite_failed_logins_total Total failed login attempts\n")
	fmt.Fprintf(w, "# TYPE vaultlite_failed_logins_total counter\n")
	fmt.Fprintf(w, "vaultlite_failed_logins_total %d\n", stats.FailedLogins)
	fmt.Fprintf(w, "# HELP vaultlite_active_connections Current active connections\n")
	fmt.Fprintf(w, "# TYPE vaultlite_active_connections gauge\n")
	fmt.Fprintf(w, "vaultlite_active_connections %d\n", stats.ActiveConns)
	fmt.Fprintf(w, "# HELP vaultlite_query_latency_ms Query latency in ms\n")
	fmt.Fprintf(w, "# TYPE vaultlite_query_latency_ms gauge\n")
	fmt.Fprintf(w, "vaultlite_query_latency_ms %f\n", stats.AvgLatencyMs)
	fmt.Fprintf(w, "# HELP vaultlite_goroutines Number of goroutines\n")
	fmt.Fprintf(w, "# TYPE vaultlite_goroutines gauge\n")
	fmt.Fprintf(w, "vaultlite_goroutines %d\n", stats.Goroutines)
	fmt.Fprintf(w, "# HELP vaultlite_memory_alloc_mb Allocated memory\n")
	fmt.Fprintf(w, "# TYPE vaultlite_memory_alloc_mb gauge\n")
	fmt.Fprintf(w, "vaultlite_memory_alloc_mb %f\n", stats.AllocMB)
	for _, db := range stats.Databases {
		fmt.Fprintf(w, "vaultlite_database_size_bytes{database=\"%s\"} %d\n", db.Name, db.Size)
	}
}
